...
openlog("DLLAV4", "pid", $syslogFacility);
...
#  Prompt    => '/(?m:^[\w.-]+\s?(?:\((dhcp-config|config)[^\)]*\))?\s?[\$#>]\s?(?:\(enable\))?\s*$)/',
  my $ssh = Net::SSH::Perl->new($host) or croak();
  $ssh->login($id, $passwd);
...
0019.d2be.2300 192.168.20.27   unknown       -               self           Assoc
...
MAC Address    IP address      Device        Name            Parent         State
0023.762b.5b00 192.168.20.29   unknown       -               self           Assoc
0025.bc07.bd00 192.168.20.23   unknown       -               self           Assoc
